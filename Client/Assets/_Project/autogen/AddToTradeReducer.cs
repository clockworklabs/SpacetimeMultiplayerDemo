...
namespace SpacetimeDB
{
	...
	{
		public static event Action<ClientApi.Event.Types.Status, Identity, ulong, uint, uint> OnAddToTradeEvent;

		public static void AddToTrade(ulong participantEntityId, uint sourcePocketId, uint destPocketId)
		{
		...
		[ReducerEvent(FunctionName = "add_to_trade")]
		public static void OnAddToTrade(ClientApi.Event dbEvent)
		{
			if(OnAddToTradeEvent != null)
			{
				...
				OnAddToTradeEvent(dbEvent.Status, Identity.From(dbEvent.CallerIdentity.ToByteArray()), args_0, args_1, args_2);
			}
		}
		[DeserializeEvent(FunctionName = "add_to_trade")]
		public static object[] AddToTradeDeserializeEventArgs(ClientApi.Event dbEvent)
		{
			var bsatnBytes = dbEvent.FunctionCall.ArgBytes;
			using var ms = new System.IO.MemoryStream();
			ms.SetLength(bsatnBytes.Length);

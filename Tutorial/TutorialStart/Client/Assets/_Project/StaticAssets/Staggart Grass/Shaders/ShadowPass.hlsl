...
Varyings ShadowPassVertex(Attributes input)
{
	...
	WindSettings wind = PopulateWindSettings(_WindAmbientStrength, _WindSpeed, _WindDirection, _WindSwinging, AO_MASK, _WindObjectRand, _WindVertexRand,
		_WindRandStrength, _WindGustStrength, _WindGustFreq, _WindPinningStrength, _WindPinningFirmness);
...
half4 ShadowPassFragment(Varyings input) : SV_TARGET
{
	...
	AlphaClip(alpha, _Cutoff, input.positionCS.xyz, input.positionWS.xyz, 1);

	half ditherVal;
	Unity_Dither_float(clamp(_LodFade, 0, 1) * 2, input.positionCS, ditherVal);
	clip(alpha - max(_Cutoff, ditherVal));
#endif
	return 0;
}